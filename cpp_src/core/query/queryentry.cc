		...
		if constexpr (injectionDirection == InjectionDirection::FromMain) {
			injectedCount +=
				injectConditionsFromOnCondition(position + injectedCount, jEntry.RightFieldName(), jEntry.LeftFieldName(), condition,
												joinedQueryEntries, QueryEntry::InjectedFromMain, joinedQueryNo, nsFrom);
		} else {
			injectedCount +=
				injectConditionsFromOnCondition(position + injectedCount, jEntry.LeftFieldName(), jEntry.RightFieldName(), condition,
												joinedQueryEntries, joinedQueryNo, QueryEntry::InjectedFromMain, nsFrom);
		}
	}
...
size_t QueryEntries::injectConditionsFromOnCondition(size_t position, const std::string &fieldName, const std::string &joinedFieldName,
													 ...
													 size_t injectingInto, const NamespaceImpl *nsFrom) {
	switch (condition) {
		case CondEq:
		case CondSet:
		case CondAllSet:
		...
		switch (condition) {
			...
			case CondLt:
				switch (entryCondition) {
					...
					case CondSet:
					case CondAllSet: {
						if (!qe.IsFieldIndexed() || !nsFrom) {	// TODO relax compare for not indexed after !1438 merge
							break;
						}
						const CollateOpts &collate = nsFrom->Indexes()[qe.IndexNo()]->Opts().collateOpts_;
